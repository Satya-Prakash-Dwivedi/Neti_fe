    ...
    <div className="bg-white min-h-screen py-12 px-6">
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-12 text-center md:text-left flex justify-between items-center">
          <div>
            <h1 className="text-3xl md:text-5xl font-lora font-black bg-gradient-to-r from-slate-700 to-slate-900 bg-clip-text text-transparent tracking-tight mb-4">Recall Hub</h1>
          ...
          <Link to="/recall/test/history" className="px-6 py-2.5 bg-slate-700 text-white font-bold rounded-full text-xs shadow-md hover:bg-slate-800 transition-colors">
            ...
            {categories.length === 0 ? (
              ...
              categories.map((category, idx) => (
                <Link 
                  ...
                  className="bg-white rounded-3xl border border-slate-200 p-8 flex flex-col items-center justify-center gap-4 hover:shadow-xl hover:-translate-y-2 hover:border-slate-400 transition-all duration-300 ease-out group"
                >
                  {category.cover_image ? (
                    ...
                    <div className="w-16 h-16 bg-slate-200 text-slate-700 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform">
                      <BookOpen className="w-8 h-8" />
                    </div>
                  )}
                  <h3 className="text-xl font-bold text-slate-800 text-center">{category.subject}</h3>
                  <div className="mt-2 text-xs font-bold text-slate-500 flex items-center gap-1 group-hover:text-slate-700 transition-colors">
